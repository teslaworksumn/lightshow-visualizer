...
public class ConfigXML {
	...
	public ConfigXML(String configFilename) throws DocumentException {
		...
		targetFilename = config.selectSingleNode("/vixenlights_viz_cfg/vltarget").getText();
		
		// Channel count
		Element channels = (Element) config.selectSingleNode("/vixenlights_viz_cfg/channels");
		channelCount = Integer.parseInt(channels.attributeValue("count"));
		
		// Window size
		Element sizeElement = (Element) config.selectSingleNode("/vixenlights_viz_cfg/size");
		...
		List<Node> shapeNodes = config.selectNodes("/vixenlights_viz_cfg/elements/shape");
